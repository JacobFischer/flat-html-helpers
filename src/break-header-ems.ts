...
/**
 * Inserts <br> tags into <hn> header tags before any <em> tags.
 *
...
export function breakHeaderEms(
    html: string,
    options: { noEndTag?: boolean } & ({ level?: number } | { levels?: number[] })
    = {},
): string {
    const br = options.noEndTag
        ? "<br>"
        : "<br />";
    ...
    for (const level of levels) {
        ...
        formatted = formatted.replace(regex, (s) => s.replace(/<em/g, (sub) => br + sub));
    }
